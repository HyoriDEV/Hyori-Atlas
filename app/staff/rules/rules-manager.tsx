...
export function RulesManager({ initialSections }: { initialSections: RuleSection[] }) {
  ...
  async function handleAddSection() {
    setIsLoading(true);
    try {
      await createRuleSection("Nouveau volet");
      toast.success("Volet ajouté");
    } catch {
  ...
  async function handleDeleteSection(id: string) {
    if (!confirm("Voulez-vous vraiment supprimer ce volet et tout son contenu ?")) return;
    setIsLoading(true);
    try {
      await deleteRuleSection(id);
      toast.success("Volet supprimé");
    } catch {
  ...
  // Calculate dynamic indexes for display
  let globalArticleIndex = 0;

  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-end">
        <Button onClick={handleAddSection} disabled={isLoading} className="gap-2">
                    ...
                    <div className="flex flex-1 items-center gap-4">
                      <div className="flex flex-col gap-1">
                        <Button
                          variant="ghost"
                          size="icon-xs"
                          onClick={() => handleMove("section", initialSections, sIndex, "up")}
                        ...
                        <Button
                          variant="ghost"
                          size="icon-xs"
                          onClick={() => handleMove("section", initialSections, sIndex, "down")}
                          disabled={sIndex === initialSections.length - 1 || isLoading}
                        >
                          <CaretDown />
                        </Button>
                      </div>
                            ...
                            <div className="flex flex-1 items-center gap-4">
                              <div className="flex flex-col gap-1">
                                <Button
                                  ...
                                  size="icon-xs"
                                  onClick={() =>
                                    handleMove("article", section.articles, aIndex, "up")
                                  }
                                ...
                                <Button
                                  ...
                                  size="icon-xs"
                                  onClick={() =>
                                    handleMove("article", section.articles, aIndex, "down")
                                  }
                                ...
                                <div key={rule.id} className="flex gap-4">
                                  <div className="flex flex-col gap-1 pt-1">
                                    <Button
                                      ...
                                      size="icon-xs"
                                      onClick={() =>
                                        handleMove("item", article.rules, rIndex, "up")
                                      }
                                    ...
                                    <Button
                                      ...
                                      size="icon-xs"
                                      onClick={() =>
                                        handleMove("item", article.rules, rIndex, "down")
                                      }
                                  ...
                                  <div className="flex flex-1 flex-col gap-2">
                                    <div className="flex items-start gap-2">
                                      <span className="text-muted-foreground mt-0.5 font-mono text-sm font-semibold">
